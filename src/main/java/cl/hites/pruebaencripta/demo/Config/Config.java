package cl.hites.pruebaencripta.demo.Config;
...
public class Config {
    ...
    private static String bundle = "encriptacion.properties";
    ...
    public Config(){
        final org.springframework.core.io.Resource[] possiblePropertiesResources = {
...
//                ,new PathResource(getCustomPath())
        ...
        try {
            final org.springframework.core.io.Resource propertiesResource = stream(possiblePropertiesResources).filter(Resource::exists).reduce((previous, current) -> current).get();
            userProperties = new Properties();
            inputStream = propertiesResource.getInputStream();
            userProperties.load(inputStream);
        }catch (NoSuchElementException e){
            String message = "No se encuentra el archivo de configuracion: "+bundle;
